...
def main(script, data, dataset, out_dir, batch_size=16, lr=0.001, wandb_project=None):
    ...
    for success_threshold in thresholds:
        for heuristic_step_check in step:
            ...
            sp.run(f'python {script} --data {data} --dataset {dataset} --out_dir {out} --batch_size {batch_size} --lr {lr} --success_threshold {success_threshold} --heuristic_step_check {heuristic_step_check} --wandb_project {wandb_project}', shell=True)
